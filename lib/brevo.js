...
const REQUEST_STATUS_COPY = {
  ...
  completed: {
    ...
    line: "Your service has been marked complete. We'd love to hear how it went — you can leave a review from your dashboard.",
  },
  cancelled: {
...
function requestDetailsBox({ ticketNumber, serviceType, technicianName }) {
  const row = (iconName, label, value) => `
    <table role="presentation" cellpadding="0" cellspacing="0" style="margin-bottom:14px;">
      <tr>
        <td style="padding-right:12px; vertical-align:top;">
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:22px 0; border:1px solid #F0D98A; border-radius:16px;">
      <tr>
        <td style="padding:20px; text-align:left; vertical-align:top;">
          ...
          ${technicianName ? row("bell", "TECHNICIAN", technicianName) : ""}
        </td>
      </tr>
    </table>
  `;
...
export async function sendServiceRequestStatusEmail({ to, name, ticketNumber, serviceType, status, technicianName }) {
  ...
  const html = emailLayout({
    ...
    body: `
      ...
      ${requestDetailsBox({ ticketNumber, serviceType, technicianName })}
