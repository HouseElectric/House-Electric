...
const POINT_ICONS = [ShieldIcon, ReportIcon, ClockIcon];
const SEGMENT_ICONS = [HomeIcon, BuildingIcon, BuildingIcon];

export default function HomeAmc() {
  const { amc } = useHomeContent();
      ...
      <div className="relative z-[1] mx-auto grid max-w-wrap grid-cols-1 items-center gap-10 px-6 md:grid-cols-[1.1fr_1fr] md:gap-14">
        <Reveal>
          ...
          <h2 className="mb-4 text-[clamp(1.75rem,3.4vw,2.6rem)] font-extrabold leading-tight text-white">
            {amc.titlePlain} <span className="text-yellow">{amc.titleHighlight}</span>
          </h2>
          <p className="mb-7 max-w-[50ch] text-[15px] leading-relaxed text-white/60">{amc.subtitle}</p>
